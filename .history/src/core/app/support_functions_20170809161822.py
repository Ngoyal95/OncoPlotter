...
def import_plot_data(file_path):
    ...
    pprint(waterfall_data)

    spider_headers = list(df_spider_data) #use to create seperate lists stored in a dict, keys are names of col headers
    spider_data = {}
    for header in spider_headers:
        spider_data[header] = df_waterfall_data[header]
