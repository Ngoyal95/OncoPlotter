...
class Waterfall(QWidget, waterfall.Ui_Waterfall):
    ...
    def add_items(self):
        ...
        for rect in self.rectangles_received:
            ...
            self.tree.setItemWidget(self.rect_item, 4, CustomCombo(self,self.keys_and_colors,rect.get_label()))
...
class WaterfallPlotter(QWidget):
    ...
    def plot(self):
        ...
        if self.settings_update == False:
            ...
            self.rects = self.ax.bar(self.rect_locations, self.waterfall_data['Best response percent change'], color=self.bar_colors,label=self.bar_colors)
