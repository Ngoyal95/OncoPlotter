...
class SwimmerPlotter(QWidget):
    ...
    def default_plot(self):
        ...
        stack_lists = [x for x in self.swimmer_data.ix[:,1:6]
        for i in range(len(stack_lists)):
            ax.barh(bar_locations, stack_length_lists[i], bar_width, color = stack_color_lists[i], left = offset_list, edgecolor = 'k')
