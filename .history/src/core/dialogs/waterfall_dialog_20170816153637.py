...
class Waterfall(QWidget, waterfall.Ui_Waterfall):
    ...
    def add_items(self):
        ...
        for rect in self.rectangles_received:
            ...
            self.tree.setItemWidget(self.rect_item, 4, CustomCombo(self,self.keys_and_colors,self.keys_received[col])) #send the label of the rectangle as the key to determine color
...
class WaterfallPlotter(QWidget):
    ...
    def plot(self):
        ...
        self.ax.grid(color = 'k', axis = 'y', alpha=0.25)
        self.canvas.draw()
        print(self.bar_colors)
        self.generated_rectangles_signal.emit([self.rects, self.bar_colors])
